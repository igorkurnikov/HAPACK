...
if(script_status == SCRIPT_START):
  outf = open("HB_PDZ1_NDSLL_44ns.dat","w")
    ...
    while(atm_tr_acceptor):
	 ...
	 continue
       if (res_d == res_a):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue
       if (res_name_d == "LYS" and res_name == "ARG"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue
       if (res_name_d == "LYS" and res_name == "GLU"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue
       if (res_name_d == "LYS" and res_name == "HIS"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue      
       if (res_name_d == "LYS" and res_name == "ASP"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue   	
       if (res_name_d == "HIS" and res_name == "GLU"):	
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue
       if (res_name_d == "HIS" and res_name == "LYS"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue  
       if (res_name_d == "ARG" and res_name == "ASP"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue  
       if (res_name_d == "ARG" and res_name == "GLU"):
        atm_tr_acceptor = AtomIteratorMolSet_GetNextAtom(atmitr_acc)
	continue  	
...
#   print "%8.3f %8.3f %8.3f " % (dist1,dist2,ang)
    if( dist1 < 3.6 and dist2 < 2.6 and ang > 1.57 and ang < 3.14):
      on_arr[i] = on_arr[i] + 1   
