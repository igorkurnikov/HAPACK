...
base class. 
We again have to restrict the recursion to $\mu_{tot} = 1$ setting
$mumin = mumax = 1$ when calling \name{logic}.
The \name{aux} parameter is used and initialized to $0$.
*/
#include "parallel.h"
#include "io_incl.h"
#include <assert.h>
...
SpinOrbRec::SpinOrbRec(Coefficient_Set& CSet,int momentum1,int momentum2,
                 Location& loc,Recursion* rec) : 
                 Recursion(CSet,momentum1,momentum2,momentum1+momentum2+1,1,
			   loc,rec)
{
  /* set the f-values */
...
\index{SpinOrbRec, ss_integ}
This is just the nuclear attraction  ss function, which is called by mulogic.
*/
void SpinOrbRec::ss_integ(const int aux,const int mu)
{
  Momentum zero(0);
  if (debug)
    cout << "  SS-Integ: ";
...
\index{SpinOrbRec, speclogic} 
We have to make sure, that the overlap integrals required in
\ref{SpinSpec} are provided.
*/
void SpinOrbRec::speclogic (const int ltot1,const int ltot2,const int aux,const
			 int mu)
{
  if(ltot1 > ltot2)
  {
    if(ltot1 - 1 > 0)
    {
      auxop -> logic(ltot1-1,ltot2,aux+1,0);
      if(ltot2 - 1 > 0)
	auxop -> logic(ltot1-1,ltot2-1,aux+1,0);
    }
  }
  else
  {  
    if(ltot2 - 1 > 0)
    {
      auxop -> logic(ltot1,ltot2-1,aux+1,0);
      if(ltot1 - 1 > 0)
	auxop -> logic(ltot1-1,ltot2-1,aux+1,0);
    }
  }
...
\index{SpinOrbRec, special}
This finally adds the speccial terms of \ref{SpinSpec} using
several specific \name{CoefArrays}. Here we should mention a point
 ...
 */
void SpinOrbRec::special(IntegArray* target,const int position,
		      const Direction& dir,
		      ...
		      const Momentum& mu,const int aux)
{
  ...
  if( a1.l() >= 0 && b1.l() >= 0 )
  {
    Direction mudir;
    mudir = mu.decrease();
    Coef_Type ct0 = (position) ? ZETAb : ZETAa;

    IntegArray* term0 = auxop -> find(a1,b1,aux+1,zero);
    assert(term0 != 0);
...
#endif
    {
      Coef_Type ct1 = (position) ? (Coef_Type) (ZETAb_BX + k)
	                : (Coef_Type) (ZETAa_AX+ k);
	...
	for(i = 0; i < coef_set[ct1].size();i++)
	  (ostream&)	  cout <<  i << coef_set[ct1].base[i]<< endl;
	cout << "       Source IntegArray " << endl
	     << "        a1: " << a1 << " b1:" << b1 << " aux: " << aux+1 
             << " mu : " << zero 
             << " INDEX (NUCLEAR): "<< compute_index(a1,b1,aux+1,zero)
             << endl;
	for(i = 0; i < term0->size();i++)
	   (ostream&) cout <<  i << term0->base[i] << endl;
	...
	for(i = 0; i < coef_set[ct0].size();i++)
	  (ostream&)	  cout << i << coef_set[ct0].base[i] << endl;
	cout << "       Source IntegArray " << endl
	     << "        a1: " << a1 << " b1:" << b1 << " aux: " << aux+1 
             << " mu : " << zero 
             << " INDEX (NUCLEAR): "<< compute_index(a1,b1,aux+1,zero)
	...
	a1.reduce(k);
      ...
      if( a1(k) >= 0 && b1(k) >= 0 )
      {
	term1     =  auxop -> find(a1,b1,aux+1,zero);
        assert(term1 != 0);
	target -> recursion_term(sign*EpsOp(dir,k,mudir)*fac,*term1);
	...
	if (debug)
	{
	  ...
	  cout << "       Source IntegArray " << endl
	    ...
	    (ostream&)	    cout << i << term1->base[i] << endl;
	  cout << "    Resulting Target IntegArray " << endl;
	  for(i = 0; i < target->size();i++)
	    (ostream&)	    cout <<  i << target->base[i] << endl;
	}
...
\index{SpinOrbRec, mulogic}
We create the nuclear attraction ss integral and call mureduce to 
calculate the appropriate coefficients. 
*/
void SpinOrbRec::mulogic (const int ltot1,const int ltot2,const int aux,const
			 int mu)
{
  ss_integ(aux+1,0);
  mureduce(ltot1,ltot2,aux,1);
}

/*TEX
\subsection{Moment Reduction}
\index{SpinOrbRec, mureduce}
Here only the coefficients due to \ref{SpinMured} have to be collected.
*/
void SpinOrbRec::mureduce(const int l1,const int l2,const int aux,
			 const int mutot)
{
  /* l1 and l2 are expected to be zero */
  ...
  {
    if(debug)
      cout << " *** MUREDUCE SPINORB *** l1: " << l1 << " l2: " << l2 
           << " aux: " << aux << " mu: " << mu << endl;
...
//			    coef_set[ZETASUM].size2());
    ...
    // term  4 * ZETAa *  ZETAb * [(A-C) X (B-C)]_i (0|0)
    ...
    ct       = (Coef_Type) (ZETAab_AxBX + dir);
    double cfac;
    
    term     =  find(l1,l2,aux+1,zero);
    assert(term != 0);
    ...
    if (debug)
    {
	 ...
	 (ostream&) cout <<  i << coef_set[ct].base[i] << endl ;
      cout << "       Source IntegArray "<< endl
           << "         l1: " << l1 << " l2: " << l2
           << " aux: " << aux+1 << " mu: " << zero 
           << " INDEX: " << compute_index(l1,l2,aux+1,zero)
           << endl;
    ...
    ct       = (Coef_Type) (ZETAab_AminusBX + dir1);
    cfac     = center(dir2);    
    ...
    ct       = (Coef_Type) (ZETAab_AminusBX + dir2);
    cfac     = center(dir1);    
    target -> recursion_term(sign*cfac,coef_set[ct],*term);
