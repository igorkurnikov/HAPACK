     ...
     1 lgu,locr,sup,rcom,homo,homo2,homo3,diep,link,sum,
...
c------------------------------------------------------------------set variables
          del=set(is,l) - sap(k) ! difference between required(set) and current values of a backbone variable
          sap(k)=set(is,l)
          if(id.eq.ioff) then
          ...
          rx=rx/rr   ! r -normalized axis of rotation to change atom coordinates   
          ry=ry/rr   !    when the valence (or torsional) angle changes
          rz=rz/rr
          x0=corm(ib,1)
          y0=corm(ib,2)
          z0=corm(ib,3)
          jl=nuc(is)-nuc(is-1)  ! number of atoms of a given residue
c
c  set coordinates of the given residue corresponding to a change of the given independent 
c  variable
c
          do j=1,jl  ! 
            m=iap(l,j,in)
...
c---------------------------------------------calculate remaining dep and gauche
      ...
      do is=1,ntl
        ...
        do l=kap(1,in)+nsv+1,kap(2,in)
            ...
            ix=nap(l,5,in)
            sapo=sap(k)
            if(id.eq.ioff) then ! if valence angle
              if(idir.gt.0) then
