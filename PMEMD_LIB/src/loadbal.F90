...
subroutine do_load_balancing(new_list, atm_cnt, crd, frc, atm_owner_map, my_atm_lst, my_atm_cnt, mass)
...
! Formal arguments:
  ...
  if (fft_slab_redist_needed) then
    ...
    if (fft_slab_redist_ctr .gt. 0) call do_fft_slab_redistribution(atm_cnt, crd, frc, atm_owner_map, my_atm_lst, my_atm_cnt,mass)
    img_redist_ctr = 0
    ...
    call do_atm_redistribution(atm_cnt, crd, frc, atm_owner_map, my_atm_lst, my_atm_cnt, mass)
...
subroutine do_fft_slab_redistribution(atm_cnt, crd, frc, atm_owner_map, my_atm_lst, my_atm_cnt, mass )
...
! Local variables:
  ...
  call mpi_gather(my_node_dat, 5, mpi_integer, gbl_loadbal_node_dat, 5, &
                  mpi_integer, 0, lib_mpi_comm, err_code_mpi)
...
! BEGIN DBG
!     write(iunit_debug,*)'DBG: avg_recip_nodes_time=', &
!                     avg_recip_nodes_time
!     write(iunit_debug,*)'DBG: last_avg_recip_nodes_time=', &
!                     last_avg_recip_nodes_time
!     write(iunit_debug,*)'DBG: recip_slab_time*max_xy_slabs=',&
!                     recip_slab_time*dble(max_xy_slab_cnt)
!     write(iunit_debug,*)'DBG: avg_total_time=',avg_total_time
!     write(iunit_debug,*)'DBG: elapsed steps=',&
!                     loadbal_step_ctr - last_loadbal_step_ctr + 1
! END DBG
    ...
    call mpi_bcast(fft_slab_dat, 6, mpi_integer, 0, lib_mpi_comm, &
                   err_code_mpi)

    if (loadbal_verbose .gt. 1 .and. recip_workload .gt. 0.d0) then
      write(iunit_debug, '(2x,a,f5.1,a)') 'Recip force workload estimate is ', &
  ...
  if (recip_numtasks .eq. numtasks) then
    call divide_images_evenly(gbl_img_div_tbl)
  else
    call divide_images_recip_biased(recip_workload, gbl_img_div_tbl)
  end if
  call do_atm_redistribution(atm_cnt, crd, frc, atm_owner_map, my_atm_lst, my_atm_cnt, mass)
...
!    loadbal_verbose = 3
    ...
    if (loadbal_verbose .gt. 2 ) then
      ...
      write(iunit_debug, 20) 'Pairlist builds done between calls: ', &
                         my_send_atm_cnts_sums
      write(iunit_debug, 30) &
        'Direct force time (in units of 100 usec) for each task:'
      write(iunit_debug, 40) (gbl_loadbal_node_dat(1, i), i = 0, numtasks - 1)
      write(iunit_debug, 30) &
        'Reciprocal force time (in units of 100 usec) for each task:'
      write(iunit_debug, 40) (gbl_loadbal_node_dat(2, i), i = 0, numtasks - 1)
      write(iunit_debug, 30) &
        'Bond-Angle-Dihedral force time (in units of 100 usec) for each task:'
      write(iunit_debug, 40) (gbl_loadbal_node_dat(3, i), i = 0, numtasks - 1)
#ifdef LISTBLD_USED
      write(iunit_debug, 30) &
        'List build time (in units of 100 usec) for each task:'
      write(iunit_debug, 40) (gbl_loadbal_node_dat(4, i), i = 0, numtasks - 1)
#endif /* LISTBLD_USED */
      write(iunit_debug, 30) &
        'Total force time (in units of 100 usec) for each task:'
      write(iunit_debug, 40) &
       (gbl_loadbal_node_dat(1, i) + &
        gbl_loadbal_node_dat(2, i) + &
        gbl_loadbal_node_dat(3, i) + &
        gbl_loadbal_node_dat(4, i), i = 0, numtasks - 1)

    end if  ! if( loadbal_verbose .gt. 2 )
...
!                 last_send_atm_cnts_total
      ...
      end if  ! if(recalc_last_send_atm_cnts_total)
...
!       write(0,*)'| DBG: ATOM REDISTRIBUTION DEBUGGING ENABLED!'
    ...
    end if  ! if( img_redist_ctr .eq. img_redist_trigger_max * nrespa / img_redist_trigger_divisor )
    ...
    if (loadbal_verbose .gt. 0) then
        write(iunit_debug, 110) &
          'Image Distribution at run step ', loadbal_step_ctr, ':'
        write(iunit_debug, 120) 'Count of images assigned to each task:'
        write(iunit_debug, 130) &
...
130 format('    ', 8i9)
  ...
  call mpi_barrier(lib_mpi_comm,err_code_mpi)
...
subroutine do_atm_redistribution(atm_cnt, crd, frc, atm_owner_map, my_atm_lst, my_atm_cnt, mass)
...
#else
  call set_minimum_mpi_bufs_size(max(3 * atm_cnt,3 * my_atm_cnt * (numtasks-1)))
#endif
...
! Local variables:
  ...
  if (ntp .eq. 0 .or. iamoeba .ne. 0) then  ! Constant volume.
  ...
  if (master ) then
    if (loadbal_verbose .gt. 0) then
        write(iunit_debug, 10) &
          ...
          loadbal_step_ctr, ':'
        write(iunit_debug, 20) 'Count of atoms assigned to each task:'
        write(iunit_debug, 30) (gbl_vec_rcvcnts(i)/3, i = 0, numtasks - 1)
    end if
  end if
...
subroutine divide_images_recip_biased(recip_workload, div_tbl)
...
! Local variables:
  ...
  if (recip_numtasks .eq. numtasks) then
    call divide_images_evenly(div_tbl)
    return
  end if
...
40 format('    ', 8i9)
  ...
  if (master) then
    ...
    if (loadbal_verbose .gt. 2) then
      ...
      write(iunit_debug, 20) 'Recip force workload assigned to ', recip_numtasks, &
                         ' tasks'
      write(iunit_debug, 22) 'Recip force workload estimate is ', &
            recip_workload * 100.d0, ' percent'
    end if
    ...
    if (loadbal_verbose .gt. 0 ) then
        write(iunit_debug, 10) &
          'Image Distribution at run step ', loadbal_step_ctr, ':'
        write(iunit_debug, 30) 'Count of images assigned to each task:'
        write(iunit_debug, 40) (div_tbl(i + 1) - div_tbl(i), i = 0, numtasks - 1)
    end if
...
subroutine divide_images_evenly(div_tbl)
...
! Local variables:
  ...
  if (master) then
    if (loadbal_verbose .gt. 0 ) then
        write(iunit_debug, 10) &
          'Image Distribution at run step ', loadbal_step_ctr, ':'
        write(iunit_debug, 20) 'Count of images assigned to each task:'
        write(iunit_debug, 30) (div_tbl(i + 1) - div_tbl(i), i = 0, numtasks - 1)
    end if
  end if
...
integer function get_atm_redist_needed()
    
    implicit none
    
    get_atm_redist_needed = atm_redist_needed
    
    return

end function get_atm_redist_needed

integer function get_fft_slab_redist_needed()

    implicit none
    
    get_fft_slab_redist_needed = fft_slab_redist_needed
    
    return

end function get_fft_slab_redist_needed
