...
! Data that should be broadcast to slave processes from the master:
  ...
  integer, save           :: do_amoeba_bonds_flag, num_list, num_params, &
                             ftable_degree
...
subroutine am_bonds_zero_flag
  implicit none
  do_amoeba_bonds_flag = 0
  return
end subroutine am_bonds_zero_flag

!*******************************************************************************!
! Subroutine:  am_bonds_set_user_bit
!
! Description: <TBS>
!
!*******************************************************************************
...
! Formal arguments:
  ...
  call set_user_bit(do_this, do_amoeba_bonds_flag)
...
! Local variables:
  ...
  if (do_amoeba_bonds_flag .ne. proceed) return
  ...
  call am_bonds_get_ene_frc(list, force_constant, fn, dfn_darg, darg_dcrd, &
                            crd, frc)
  ene = energy
...
! Formal arguments:
  ...
  double precision, intent(out) :: darg_dcrd(3, *)
...
!*******************************************************************************!
! Subroutine:  am_bonds_get_ene_frc
!
! Description: <TBS>
!
!*******************************************************************************

subroutine am_bonds_get_ene_frc(blist, force_constant, fn, dfn_darg, &
                                darg_dcrd, crd, frc)
...
! Formal arguments:
  ...
  double precision, intent(in)          :: dfn_darg(*)
  double precision, intent(in)          :: darg_dcrd(3, *)
  double precision, intent(in)          :: crd(3, *)
  double precision, intent(in out)      :: frc(3, *)
...
! while darg_dcrdi holds the derivs of b-b0 with respect to crds of i.
    ...
    f(1) = frc_K * dfn_darg(n) * darg_dcrd(1, n)
    f(2) = frc_K * dfn_darg(n) * darg_dcrd(2, n)
    f(3) = frc_K * dfn_darg(n) * darg_dcrd(3, n)
...
subroutine set_bond_list(list_new,n)
...
end subroutine set_bond_list

subroutine set_bond_params(fc,eq,n)
...
end subroutine set_bond_params
...
subroutine set_valid_bit(ival)
    ...
    if(ival .eq. 1) then
        do_amoeba_bonds_flag = ibset(do_amoeba_bonds_flag, valid_bit)
    else
        do_amoeba_bonds_flag = ibclr(do_amoeba_bonds_flag, valid_bit)
    endif
        
end subroutine set_valid_bit

end module amoeba_bonds_mod
