...
public:
	...
	int InitForceField(std::string ff_type_str, StrStrMap options = {} ); //!< Initiate Molecular Mechanics Model with force field name 
	int InitModel(const ForceFieldType & = MMForceField::ff_type_default, StrStrMap options = {}); //!< Initiate Molecular Mechanics Model with force field name ff_name_par
	...
	// std::map<HaAtom*, shared_ptr<AtomFFParam>> atom_mut_params; //!< Atom FF parameters for the mutated state
