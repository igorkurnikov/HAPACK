...
public:
	...
	std::map<std::string, std::vector<double>>  symb_ppar_map;     //!< Map strings to VdW atom parameters
	std::map<std::string, std::vector<double>>  symb_hpar_map;     //!< Map strings to H bond atom parameters
	std::map<std::string, std::vector<double>>  bond_param_map;    //!< Map strings to Valence Bonds parameters
	std::map<std::string, std::vector<double>>  vang_param_map;    //!< Map strings to Valence Angles parameters  
	std::map<std::string, std::vector<double>>  dih_param_map;     //!< Map strings to Dihedral Angles parameters
	std::map<std::string, std::vector<double>>  impdih_param_map;  //!< Map strings to Improper Dihedral Angles Parameters

	double FindAtomMassFromSymbol( const std::string& ats );    //!< Find atom mass for atom ff symbol ( return -1.0 if not defined )
	std::vector<double>  FindPointParamFromSymbol( const std::string& ats1);  //!< Find Force Field atom parameters for FFSymbol
	std::vector<double>  FindBondParamFromSymbol ( const std::string& ats1, const std::string& ats2);
	std::vector<double>  FindHBondParamFromSymbol (const std::string& ats1, const std::string& ats2);
	std::vector<double>  FindValAngleParamFromSymbol(const std::string& ats1, const std::string& ats2, const std::string& ats3);
	std::vector<double>  FindDihedralParamFromSymbol(const std::string& ats1, const std::string& ats2, const std::string& ats3, const std::string& ats4, bool improper_flag = false);
	...
	static std::string GetAmberResName( const std::string& full_res_name, const ForceFieldType& ff_type = ForceFieldType::UNKNOWN_FF ); //!< Get Mort Library residue name for particular full residue name ( with modifier )
	static std::string GetAmberAtName(  const std::string& at_name, const std::string& full_res_name, const ForceFieldType& ff_type = ForceFieldType::UNKNOWN_FF ); //!< Get AMBER Library atom name for particular full residue name ( with modifier )
