...
// static void statistics( double isovalue, int num_cubes );
#endif
...
static void *xalloc( int size, int type )
{
//   return (void *) allocate_type( ctx, size, type);
	return (void *) malloc( size ); // Substituted just by malloc not to be dependent on Vis5D memory routines
}

// This is bogus types of variable for not to use vis5d allocation routines
#define IXPLANE_TYPE   1
#define PTFLAG_TYPE    1
#define PTAUX_TYPE     1
#define PCUBE_TYPE     1
#define POLFVERT_TYPE  1
#define NXA_TYPE       1
#define PNX_TYPE       1
#define TRISTRIPE_TYPE 1
#define VETPOL_TYPE    1


/* ----- Routine to deallocate memory space with check */

//static void xfree( Context ctx, void *pt )
static void xfree( void *pt )
{
   if (pt != NULL)
//     deallocate(ctx, pt, -1);
	free(pt); // Substituted just by free not to be dependent on Vis5D memory routines
}
...
static int marching( float ptGRID[], int xdim, int ydim, int zdim,
                     ...
                     int Pol_f_Vert[], int Vert_f_Pol[] )
{
    ...
    */
    ...
    iz = ydim;
    ii = 2 * (ix + iy + iz);
    ixPlane[0] = (int *) xalloc( ii * sizeof(int), IXPLANE_TYPE);
                     ...
                     int Pol_f_Vert[], int Vert_f_Pol[] )
{
   ...
   minimum_area = (float)max(1.e-4,EPS_0);
...
void 
HaSurface::main_march( float *ptGRID, int NC, int NR, int NL,
                 int LOWLEV,
                 ...
                 int *ITRI)
/*
 *  INPUT: ptGRID - pointer to the grid gata
 *	       NC , NR, NL  - Number of columns, rows and levels in the grid  
 *         LOWLEV       - Lowest level in the grid
 *         GLEV  - Value of the isolevel to sunstruct isosurface
 *         ARX, ARY, ARZ - parameters of marching cubes ( = 1.0 in the calling routine)
 *         NVERTS - Maximum Number of verticies = MAX_ISO_VERTS in the calling routine
 *         NPTS   - Maximum Number of ??  = 2* MAX_ISO_VERTS in the calling routine
 *
 *  OUTPUT: Surface as a triangle strip : connected triangles verticies define triangles as 1-2-3, 2-3-4, 4-5-6 etc  
 *          NVERTS - Number of verticies = MAX_ISO_VERTS in the calling routine
 *          VX, VY, VZ - Coordinates of verticies in grid coordinates 
 *          NX, NY, NZ - Normal vectors ( from -1.0 to 1.0) at the verticies
 *          VPTS -  indexes of points ?? 
 *          IVERT - The number of verticies found
 *          IPTS  - The number of indexies found
 *          
 *
 */
{
   ...
   int      *Pol_f_Vert, *Vert_f_Pol;
	...
	arX = ARX;    arY = ARY;    arZ = ARZ;
        ...
        ptFLAG = (int *) xalloc(  num_cubes * sizeof(int), PTFLAG_TYPE );
        ptAUX = (int *) xalloc(  xdim_x_ydim_x_zdim * sizeof(int), PTAUX_TYPE );
        pcube = (int *) xalloc( (num_cubes + 1) * sizeof(int), PCUBE_TYPE );
	...
	if (npolygons <= 0) {
       ...
       iy = 7 * npolygons;
       ii = ix + iy;
       Pol_f_Vert = (int *) xalloc ( ii * sizeof(int), POLFVERT_TYPE );
       if (!Pol_f_Vert) {
          xfree( ptFLAG );
          *IVERT = *IPTS = *IPOLY = *ITRI = 0;
          return;
       ...
       nvertex = marching(  ptGRID, xdim, ydim, zdim, ptFLAG, VX, VY, VZ,
	...
	if (nvertex <= 0) {
        ...
        NxA = (float *) xalloc(  6 * npolygons * sizeof(float), NXA_TYPE );
        NxB = NxA + npolygons;
        NyA = NxB + npolygons;
	NyB = NyA + npolygons;
	NzA = NyB + npolygons;
	NzB = NzA + npolygons;
	Pnx = (float *) xalloc(  3 * npolygons * sizeof(float), PNX_TYPE );
	Pny = Pnx + npolygons;
	Pnz = Pny + npolygons;
        if (!NxA || !Pnx) {
           xfree(  NxA );
	...
	*/
	...
	if (arZ != 1.0)
        ...
        Tri_Stripe = (int *) xalloc( 6*npolygons * sizeof(int), TRISTRIPE_TYPE );
        vet_pol = (int *) xalloc(  npolygons * sizeof(int), VETPOL_TYPE );
	...
	/* ----- Realese Memory */
        xfree(  Pol_f_Vert);	/* Free Pol_f_Vert, Vert_f_Pol*/
        xfree(  vet_pol );
        xfree(  Tri_Stripe );
