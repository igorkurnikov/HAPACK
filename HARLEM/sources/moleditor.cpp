...
int MolEditor::RenameAtomsFlexToAmber(HaResidue* pres)
{
	HaResDB* p_res_db = HaResDB::GetDefaultResDB();

	try
	{
		if (pres->size() == 0) return FALSE;
		HaAtom* aptr = (*pres)[0];
		if (!aptr->Selected()) return FALSE;
		if (!pres->IsNucleo()) return FALSE;
		std::string res_name = pres->GetName();
		std::string name_mod = pres->GetNameModifier();

		if (res_name == "A" || res_name == "G" || res_name == "C" || res_name == "T")
		{
			...
			HaAtom* ph1n2 = pres->GetAtomByName("H1N2"); // G
			HaAtom* ph2n2 = pres->GetAtomByName("H2N2"); // G
			...
			{
				ph1c5->SetName("H71");
				PrintLog("In Residue %s  Rename H1C5->H71 \n", pres->GetRef().c_str());
			}
			if (ph2c5)  // T
			{
				ph2c5->SetName("H72");
				PrintLog("In Residue %s  Rename H2C5->H72 \n", pres->GetRef().c_str());
			}
			if (ph3c5)  // T
			{
				ph3c5->SetName("H73");
				PrintLog("In Residue %s  Rename H3C5->H73 \n", pres->GetRef().c_str());
			}
			if (ph1n2)  // G
			{
				ph1n2->SetName("H21");
				PrintLog("In Residue %s  Rename H1N2->H21 \n", pres->GetRef().c_str());
			}
			if (ph2n2) // G
			{
				ph2n2->SetName("H22");
				PrintLog("In Residue %s  Rename H2N2->H22 \n", pres->GetRef().c_str());
			}
