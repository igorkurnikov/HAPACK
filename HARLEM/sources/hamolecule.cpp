...
bool HaMolecule::AddMolCopy(HaMolecule& Mol_ref, bool create_new_chain, AtomAtomMap* ptr_atom_map)
{
	...
	std::map<int,HaResidue*>::iterator res_itr;
...
int HaMolecule::AttachFragment(HaAtom* catom_host, HaAtom* catom_frag )
{
	...
	std::map<HaAtom*, HaAtom*>::iterator maitr;
...
{
	auto p_opt = std::unique_ptr<AtomSaveOptions>( p_opt_arg ? (AtomSaveOptions*) p_opt_arg->clone() : new AtomSaveOptions() );
...
{
	std::unique_ptr<AtomSaveOptions> p_opt(p_opt_arg != NULL ? (AtomSaveOptions*)p_opt_arg->clone() : new AtomSaveOptions());
	...
	sout << " GROMACS Gro file Generated by HARLEM " << "\n";
	char buf[256];
	sprintf(buf, "%5d", na);
	sout << buf << "\n";
	idx = 1;
...
HaBond* MolSet::AddBond(HaAtom* src, HaAtom* dst )
{
	...
	std::shared_ptr<HaBond> pb = std::make_shared<HaBond>(src,dst);
	Bonds.push_back(pb);
...
bool MolSet::DeleteBond(HaAtom* src, HaAtom* dst)
{
	...
	std::vector<std::shared_ptr<HaBond>>& bonds1 = src->GetBonds();
	std::vector<std::shared_ptr<HaBond>>& bonds2 = dst->GetBonds();

	std::shared_ptr<HaBond> bnd_d;
...
{
	std::list<HaChain>::const_iterator citr;
	int icount=0;
...
void HaMolecule::DeleteFeatures(const int itype)
{
...
{
	std::list<SecStructElement>::const_iterator fitr;
	int icount=0;
...
bool HaMolecule::SetUniqueAtomNames()
{
	...
	std::list<HaResidue*> old_groups;
	std::list<HaResidue*>::iterator ritr;
...
{
	const MolSet* pmset = GetHostMolSet();
	std::set<HaHBond>::const_iterator bitr;
	int nb = 0;
