...
public:
	...
	int DeleteExtraAtoms(HaMolSet* pmset);   //!< Delete atoms not found in residues templates
	int AddMissingAtoms(HaMolSet* pmset);    //!< Add Missing atoms in the structure using residue templates
	int AddHydrogens(HaMolSet* pmset);       //!< Add hydrogens
	int AddPolarHydrogens(HaMolSet* pmset);  //!< Add Polar hydrogens involved in hydrogen-bonding
	int AddHydrogensHybrid(HaMolSet* pmset); //!< Add Hydrogens using atom hybridization information
	int SetAtomElemFromTempl(HaMolSet* pmset){return 0;} //!< Set Atom Elements from Residue Templates
	int SetHBondDonAccStatus(AtomContainer* p_at_coll); //!< Set Hydrogen Bond donor Acceptor status for atoms in a collection
	int SetStdAtomicParams(HaMolSet* pmset, int at_params_type); //!< Set atomic parameters according to flag at_params_type 
	int SetFormalAtChrgFromTempl(HaMolSet* pmset); //!< Set Formal Atomic Charges From Residue Templates
	static int ClearAtomFFParams(HaMolSet* pmset); //!< Clear Atomic Force Field Parameters 
	int FixBondsUsingTempl(HaMolSet* pmset); //!< Fix Bonds of the selected residues according to residue templates
    ...
    bool CalcHBonds(HaMolSet* pmset, bool recalc=false);  //!< Compute Hydrogen Bonds for the molecular Set
	int IsValidHBond(HaHBond* p_hb); //!< Check if hydrogen bond satisfy current criteria 
	void CalcHydrogenBonds(HaMolSet* pmset);            //!< Compute Hydrogen Bonds for the molecular Set - Old,not used, to incorporate
	void FindDisulphideBridges(HaMolSet* pmset);             //!< Find Disulphide bridges
	...
	int FindBackbone (HaMolSet* pmset);  //!< Compute Backbone Bonds for the molecular Set
	int UpdateBackBone(HaMolSet* pmset); //!< Recompute Backbone Bonds for the molecular Set if to_find_backb flag is set
	...
	void SetAlphaHelix(HaMolSet* pmset); //!< Set selected part of the protein to an ideal alpha helix conformation
	...
	int Solvate(HaMolSet* pmset); //!< Solvate Molecular Set 
	int CenterSoluteInSolvent(HaMolSet* pmset); //!< Center solute in the center of periodic box
	int CenterMolInPBox(HaMolSet* pmset);  //!< Center Molecule in the center of periodic box
	int ReplicatePeriodBox(HaMolSet* pmset, int nx, int ny, int nz); //!< Replicate Molecular Set Along X,Y,Z axes using box periodical boundary info
    int WrapToUnitCell(AtomContainer* at_cont, PeriodicUnitInfo* per_info); //!< Wrap Atoms in the periodical system to the Unit Cell  
    int DeleteOverlapMols(HaMolSet* pmset, AtomGroup& at_coll); //!< Remove all molecules (connected groups of atoms) that overlap at_coll 
	...
	HaMolecule* CreateTransAlk(HaMolSet* pmset, const int nunit, const std::string& name="ALK");   //!< Create trans-alkane with the length of nunit   
	HaMolecule* CreateSurf(HaMolSet* pmset, const int num_layers, const std::string& name="GOLD"); //!< Create Several layers of the metal surface
	bool Create2DMolArray(HaMolSet* pmset, HaMolecule* pMol_ref, 
	...
	bool AddElectrSurf(HaMolSet* pmset, int add_surf_below_flag, int add_surf_top_flag, int add_atom_top_flag,
